...
// // Utility function to check if the contract contains the required SPDX license and pragma directive
// const isValidSolidityContract = (filename: string, code: string) => {
...
// }) => {
//   const handleAnalyze = () => {
//     const filename = "test.js"; // Replace with dynamic filename if needed

//     if (!isValidSolidityContract(filename, contract)) {
//       alert(
//         "The provided code does not appear to be a valid Solidity smart contract. Make sure it starts with the SPDX license identifier and the 'pragma' directive."
//       );
//       return;
...
import Editor from "react-simple-code-editor";
import Prism from "prismjs";
import "prismjs/components/prism-go"; // GO highlight
...
const isValidGo = (code: string) => {
  const goPattern = /(package\s+\w+|func\s+\w+\s*\(.*\)|import\s+\(|fmt\.Println\()/;
  return goPattern.test(code);
};
...
const CustomCodeEditor: React.FC<CustomCodeEditorProps> = ({
  ...
  const handleAnalyze = () => {
    if (!isValidGo(contract)) {
      alert("Only Go (Golang) code is allowed. Please enter valid Go code.");
      return;
    }
    analyze();
  };
